...
For a store that cannot reach Composer — air-gapped, or where the extension is
handed over as a file — download the archive attached to the
[latest release](https://github.com/zipMoney/zip.magento2/releases). It already
contains the Zip PHP library, so there is nothing further to install.

- Unpack it into `/yourMagento2root/app/code/`, giving you
  `app/code/Zip/ZipPayment/`
- Then `module:enable`, `setup:upgrade`, `setup:di:compile` and
  `setup:static-content:deploy` as above

Do not use a plain checkout of this repository for that — it deliberately does
not carry the library, and Magento never reads a module's `composer.json` when
the module is dropped into `app/code`. Prefer Composer where you can: it keeps
the extension and the library updatable together.
